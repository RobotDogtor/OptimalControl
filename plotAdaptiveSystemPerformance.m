...
subplot(4,1,1)
    ...
    plot(t_ode(1:pp:end),xr(1:pp:end),'r',t_ode(1:pp:end),xd(1:pp:end),'k--',t_ode(1:pp:end),x(1:pp:end),'b')
...
subplot(4,1,3)
    ...
    plot(t_ode(1:pp:end),thx(1:pp:end),'r-',[0,t_ode(end)],thx_star*[1 1],'r--',...
        t_ode(1:pp:end),thr(1:pp:end),'b-',[0,t_ode(end)],thr_star*[1 1],'b--')
    legend('\theta_x','\theta_x*','\theta_r','\theta_r*','Location','southeast')
    xlabel('Time (s)')
